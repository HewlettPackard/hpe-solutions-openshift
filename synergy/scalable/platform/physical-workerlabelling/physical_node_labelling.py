...
def label_for_security():
    """This module will label the physical nodes as per the 
    overall security posture "OK or Degraded" of the node.
    """
    user_env = set_environment_var()
    data = get_all_node_security_status()
    for key, value in data.items():
...
def main():
    options = ["Get the physical worker node details.", \
        ...
        try:
            ...
            if user_input in range(1, 9):
                if user_input == 1:
                    print(get_all_nodes())
                if user_input == 2:
                    print(get_all_nodes_health())
                if user_input == 3:
                    print(get_all_node_security_status())
                if user_input == 4:
                    print(label_for_health())
                if user_input == 5:
                    print(label_for_security())
                if user_input == 6:
                    print(custom_labelling())
                if user_input == 7:
                    display_all_node_labels()
                if user_input == 8:
                    print("Exiting!!")
            else:
                print("Invalid input!")
