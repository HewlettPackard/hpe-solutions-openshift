...
Prerequisites:
	...
	we will be using single server for all the above services and which will be having internet access.
 
Download Server requirements:
...
1.Create Yum Repo server
------------------------
a)Navigate to yum folder
  cd /opt/hpe-solutions-openshift-master/DL-LTI-Openshift/airgap/yum

b)Run the below command to create yum repo server
  ansible-playbook -i hosts playbooks/create_local_yum_repo.yaml
  
2.Mirror Registry
-----------------
a)Navigate to /opt/hpe-solutions-openshift-master/DL-LTI-Openshift/ folder and if it is airgap based deploymnt then make sure to fill the below values in input.yaml file
